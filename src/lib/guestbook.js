...
export function addReply(messages, messageId, entry) {
  const message = sanitize(entry.message, 220);
  const name = sanitize(entry.name, 40) || 'anonymous';
  if (!message) return messages;
...
if (globalThis.process?.argv?.[1]?.endsWith('guestbook.js')) {
  ...
  const replied = addReply(once, seed[0].id, { name: 'x', message: 'reply' });
  console.assert(replied[0].replies.length === 1, 'reply is appended');
}
